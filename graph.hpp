...
class Graph
{
    public:
        ...
        GraphElem global_to_local(GraphElem idx) {
            GraphElem local_offset = (idx - get_base(rank_));
            assert(local_offset < lnv_);
            return local_offset;
        }
    ...
    private:
        // Local # of vertices/edges ?
        GraphElem lnv_, lne_;
        // Global # of vertices/edges ?
        GraphElem nv_, ne_;
        /*
         * parts[rank] stores the starting vertex index of vertices stored on
         * rank. Vertices [parts_[rank], parts[rank+1]) are stored on rank.
         */
